...
$('.sign-in-manufacture').click(function() {
  ...
  //create json data
  var inputData = '{' + '"manufactureid" : "' + formmanufactureId + '", ' + '"cardid" : "' + formCardId + '"}';
  console.log(inputData);

  //make ajax call
  $.ajax({
    type: 'POST',
    url: apiUrl + 'manufactureData',
    data: inputData,
    dataType: 'json',
...
function approveDocument(documentId) {
  ...
  //create json data
  var inputData = '{' + '"manufactureid" : "' + formmanufactureId + '", ' + '"cardid" : "' + formCardId + '", "documentId" :"'+documentId+'"}';
  console.log(inputData);
    ...
    success: function(data) {
      ...
      if (data.error) {
        ...
        $('.dashboards').html(function() {
            ...
            var str = '<table width="100%"  class="blueTable  documentList" border="1" cellspacing="1" cellpadding="1"><tr><th>Retailer</th><th width="50%">Document Name </th><th width="50%">Document Status </th></tr>';
            var documentsData = data.approvedDocs;
            if(documentsData)
            {
              for (var i = 0; i < documentsData.length; i++) {
        ...
        //update dashboard
        $('.reviewandapprove').html(function() {
            var str = '<table width="100%"  class="blueTable documentList" border="1" cellspacing="1" cellpadding="1"><tr><th>Retailer</th> <th>Document Name </th> <th>Approve </th></tr>';
            var documentsData = data.approvalPendingList;
            if(documentsData)
            {
              for (var i = 0; i < documentsData.length; i++) {
